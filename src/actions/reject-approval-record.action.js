...
exports.RejectApprovalRecordAction = void 0;
const logger_1 = require("../core/logger");
const rbac_1 = require("../core/rbac");
class RejectApprovalRecordAction {
    execute(command, store) {
        ...
        if (!(0, rbac_1.canManageApprovalRecord)(command.actor, command, record)) {
            store.recordApprovalInvalidAttempt(command.recordId, command.shipId, command.issuedAt, command.actor, command.transitionId ?? null, "Actor is not authorized to reject this approval record", command.note ?? null);
        ...
        store.rejectApprovalRecord(command.recordId, command.shipId, command.issuedAt, command.actor, command.transitionId ?? `${command.type}:${command.recordId}:${command.issuedAt}`, command.reason ?? null, command.note ?? null);
    }
}
exports.RejectApprovalRecordAction = RejectApprovalRecordAction;
//# sourceMappingURL=reject-approval-record.action.js.map