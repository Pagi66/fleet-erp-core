...
exports.canManageApprovalRecord = canManageApprovalRecord;
function canExecuteAction(actor, command, task) {
    if (actor === "SYSTEM") {
        return true;
    }
    switch (command.type) {
...
function taskCompletionAllowed(actor, task) {
    if (!task) {
        return false;
    }
...
function canManageApprovalRecord(actor, command, record) {
    if (actor === "SYSTEM") {
        return true;
    }
    switch (command.type) {
        case "CREATE_APPROVAL_RECORD":
            return (command.originRole === actor &&
