...
function startHttpServer(dependencies, port = config_1.config.port) {
    const server = (0, http_1.createServer)(async (request, response) => {
        try {
            await handleRequest(request, response, dependencies);
        }
        catch (error) {
            logger_1.logger.error("http_request_failed", error, {
...
async function handleRequest(request, response, dependencies) {
    ...
    if (method === "POST" && url.pathname.startsWith("/tasks/") && url.pathname.endsWith("/complete")) {
        ...
        const completed = dependencies.completeTaskAction.execute(taskId, actor, dependencies.store);
...
function getNotificationIdFromPath(pathname) {
...
function getRecordIdFromPath(pathname) {
...
function isApprovalTransitionPath(pathname) {
    return /^\/records\/[^/]+\/(submit|approve|reject)$/.test(pathname);
}
function getRecordIdFromTransitionPath(pathname) {
...
function isValidEventType(value) {
    return (value === "DAILY_LOG_CHECK_DUE" ||
...
function isValidAssignedRole(value) {
...
function getActorContext(url, role) {
    const shipId = getOptionalShipId(url);
    if ((role === "MARINE_ENGINEERING_OFFICER" ||
...
function getAwarenessOptions(url) {
    const shipId = getOptionalShipId(url);
...
function getOptionalShipId(url) {
    const shipId = url.searchParams.get("shipId");
    return shipId && shipId.trim() !== "" ? shipId : undefined;
}
function getOptionalLimit(url) {
    return getOptionalNumber(url, "limit");
}
function getOptionalNumber(url, name) {
...
function logRejectedRequest(request, reason) {
    logger_1.logger.warn("http_request_rejected", {
        ...(request.method ? { eventType: request.method } : {}),
        result: request.url ?? "",
        status: reason,
    });
...
function extractRole(request, body) {
    const headerRole = request.headers["x-role"];
    const candidate = typeof headerRole === "string"
        ? headerRole
...
function validateApprovalCreatePayload(value, actor) {
    if (!isRecord(value)) {
...
function validateApprovalTransitionPayload(value, actor, recordId, pathname) {
    if (!isRecord(value)) {
...
function isValidRecordKind(value) {
