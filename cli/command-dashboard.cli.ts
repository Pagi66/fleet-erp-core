...
export async function runCommandDashboardCli(
  rl: CliPromptLike,
  app: CliApp,
  actor: ActorContext,
  config: DashboardCliConfig,
): Promise<void> {
  ...
  while (true) {
    ...
    if (input === "r" || input === "") {
      previousState = toPreviousState(
        dashboardState.navigation,
        dashboardState.navigation.selectedRecordId,
      );
      continue;
    }
    ...
    previousState = await openRecordDetail(
      rl,
      app,
      actor,
      selected,
...
function renderDashboard(
  ...
  renderSection(
    "ACTION REQUIRED",
    selections.filter((entry) => entry.section === "ACTION_REQUIRED"),
  );
  renderSection(
    "NEEDS ATTENTION",
    selections.filter((entry) => entry.section === "NEEDS_ATTENTION"),
  );
  renderSection(
    "FOR AWARENESS",
    selections.filter((entry) => entry.section === "FOR_AWARENESS"),
  );
}
...
function renderSection(
  ...
  for (const selection of selections) {
    ...
    console.log(
      `${selection.index}. [${card.ship.name}] ${card.title} (${formatAge(card.ageHoursSinceLastAction)})`,
    );
  }
  console.log("");
}
...
async function openRecordDetail(
  ...
  while (true) {
    clearScreen();
    const card = toCommandDashboardCardView(selection.record);
    const detailView = app.store.getApprovalRecordViewForActor(
    ...
    console.log("");
    console.log(
      `Reason: ${detailRecord?.approval.lastActionReason ?? card.reason ?? "N/A"}`,
    );
